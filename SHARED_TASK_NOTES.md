...
- `src/layouts/BaseLayout.astro` — shared layout (sidebar, header, footer, RSS link tags, JSON-LD structured data)
- `src/pages/index.astro` — homepage
- `src/pages/work/[slug].astro` — work detail (includes book cover gallery for editing page)
- `src/pages/poetry/[slug].astro` — poetry detail (with CreativeWork JSON-LD)
- `src/pages/prose/[slug].astro` — prose detail (with Article JSON-LD)
...
- BaseLayout accepts optional `jsonLd` prop — pages can pass page-specific structured data, otherwise defaults to WebSite+Person schema
